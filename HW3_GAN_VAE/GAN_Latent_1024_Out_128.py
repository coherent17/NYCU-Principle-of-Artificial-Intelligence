...
output_dir = './GAN_Latent_1024_Out_128_Generated_Images'
...
def build_generator(latent_dim):
    ...
    model.add(layers.Dropout(0.4))  # Add dropout layer to prevent overfitting
    ...
    model.add(layers.BatchNormalization())
    model.add(layers.LeakyReLU(alpha=0.2))
    model.add(layers.Conv2DTranspose(3, (4, 4), activation='tanh', padding='same', strides=(2, 2)))  # Output shape: (128, 128, 3)
    model.summary()
    return model
...
#     plt.subplot(2, 5, i + 1)
#     plt.imshow(np.reshape(predict_images[i] * 0.5 + 0.5, (128, 128, 3)))  # Rescale images to [0, 1]
#     plt.axis('off')
